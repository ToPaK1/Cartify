...
CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,address_id INTEGER,status TEXT NOT NULL DEFAULT 'Pending' CHECK(status IN ('Pending','Processing','Shipped','Delivered','Cancelled')),payment_method TEXT NOT NULL DEFAULT 'COD' CHECK(payment_method IN ('COD','MockCard')),subtotal REAL NOT NULL CHECK(subtotal>=0),shipping_fee REAL NOT NULL DEFAULT 0 CHECK(shipping_fee>=0),total REAL NOT NULL CHECK(total>=0),created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(address_id) REFERENCES addresses(id) ON DELETE SET NULL);
...
const categories = [
  ['Electronics','electronics'],['Fashion','fashion'],['Home & Living','home-living'],['Beauty','beauty'],['Sports','sports']
];
...
if (db.prepare('SELECT COUNT(*) count FROM products').get().count === 0) {
  ...
  const seed = db.transaction(() => {
    ...
    insert.run(cat('Sports'),'Training Backpack','training-backpack','Durable backpack for gym and daily training.','ActiveGear',899,1099,22,4.3,'https://images.unsplash.com/photo-1553062407-98eeb64c6a62');
  });
  seed();
}
